...
public class Condition {
    ...
    Condition(CosmosReference cosmosReference, String attribute, Object value, ComparisonOperator comparisonOperator, boolean includedIfNull, Boolean boolExpr) {
        if (cosmosReference == null)
            throw new CosmosQueryBuilderException("Collection or Join reference must be provided");
        if (attribute == null)
            throw new CosmosQueryBuilderException("attribute must be provided");
        if (comparisonOperator == null)
            throw new CosmosQueryBuilderException("comparison operator must be provided");
